...
package com.java.arobjviewer.parser;
...
import com.java.arobjviewer.models.Group;
			...
			if (line.length() > 0) {
				if (line.startsWith("#")) {
					...
					vertices.add(new float[]{
							Float.parseFloat(spaceTokenizer.next().trim()),
							Float.parseFloat(spaceTokenizer.next().trim()),
							Float.parseFloat(spaceTokenizer.next().trim())});
				}
					...
					for (int i = 0; i < 3; i++) {//only triangles supported
						...
						if(vertexCount == 2) {
							//vertex reference
							vertexID = Integer.parseInt(slashTokenizer.next().trim())-1;
							//normal reference
							normalID = Integer.parseInt(slashTokenizer.next().trim())-1;
							throw new ParseException(modelName,
									lineNum,
							...
							//vertex reference
							vertexID = Integer.parseInt(slashTokenizer.next().trim())-1;
							String texCoord = slashTokenizer.next().trim();
							if(!texCoord.equals("")) {
								//might be omitted
